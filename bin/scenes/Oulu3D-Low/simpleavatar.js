...
var commands = [1,3,1,1,1,1,5,10,6,3,1,1,11,12,8,7,4,3,1,9,10,4,5,11,12,8,4,
                1,1,1,4,9,10,8,7,5,4,2,1,11,12,5,7,11,12,1,1,1,2,6,4,3,9,10];
var index = 0;
...
SimpleAvatar.prototype.ClientInitialize = function() {
    ...
    if (this.me.name == "Avatar" + client.connectionId) {
        ...
        //this.me.Action("StartMoving").Triggered.connect(this, this.MoveAvatar);
...
SimpleAvatar.prototype.MoveAvatar = function()
{
    if (this.isServer)
        return;
    ...
    if (decision == 3)
        this.me.Exec(2, "Move", "right");
    if (decision == 4)
        this.me.Exec(2, "Move", "left");
    if (decision == 5)
        this.me.Exec(2, "Stop", "forward");
    if (decision == 6)
        this.me.Exec(2, "Stop", "back");
    if (decision == 7)
        this.me.Exec(2, "Stop", "right");
    if (decision == 8)
        this.me.Exec(2, "Stop", "left");
    if (decision == 9)
        this.me.Exec(1, "Rotate", "right");
    if (decision == 10)
        this.me.Exec(1, "StopRotate", "right");
    if (decision == 11)
        this.me.Exec(1, "Rotate", "left");
    if (decision == 12)
        this.me.Exec(1, "StopRotate", "left");

    frame.DelayedExecute(1.0).Triggered.connect(this, this.MoveAvatar);
}
